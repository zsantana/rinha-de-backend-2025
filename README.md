...
1. Tempos de resposta aumentados: o endpoint de pagamentos demora a responder.
1. Serviço indisponível: o endpoint retorna um erro de servidor HTTP 5XX e não processa pagamentos. 

Caberá a você desenvolver a melhor estratégia para processar pagamentos o mais rápido possível e com a menor taxa – esse é o objetivo principal dessa edição da Rinha. Por favor, note que é possível enviar as requisições de pagamento de forma assíncrona. Porém, quanto mais rápido você enviar os pagamentos para o Payment Processor, maior chance terá de ir bem porque ao final do teste haverá uma contagem de quantos pagamentos foram processados. Você obterá menos pontos se houver muitos pagamentos ainda não enviados aos serviços Payment Processor ao final do teste.

![alt text](misc/exemplo-proc-assinc.png)

Para facilitar a verificação da disponibildade dos serviços, para cada serviço, é fornecido um endpoint de **health-check** que mostra se o serviço está enfrentando falhas e qual é o tempo mínimo de resposta para o processamento de pagamentos. Entretanto, este endpoint possui um limite de uma chamada a cada 5 segundos. Caso este limite seja ultrapassado, uma resposta HTTP 429 - Too Many Requests será retornada. Use-o com sabedoria!

Os dois serviços Payment Processor são idênticos em termos de endpoints – a única diferença realmente é a taxa. Novamente, quanto mais requisições de processamento de pagamentos forem enviadas para o serviço **default**, menos taxa você pagará e isso será melhor para a pontuação da Rinha.

## Como Participar

As seções seguintes mostram um passo a passo para você participar da Rinha! Pessoas de todos os níveis costumam participar da Rinha de Backend – desde iniciantes até pessoas bastante experientes. O mais importante da Rinha é o espírito de cooperação entre os participantes e aprendizado. Entretanto, é recomendado que você tenha conhecimento em alguma lingugem de programação, docker, e git.
...
**resposta**
- `default.totalRequests` é um campo obrigatório do numérico inteiro.
- `default.totalAmount` é um campo obrigatório do decimal.
- `fallback.totalRequests` é um campo obrigatório do numérico inteiro.
- `fallback.totalAmount` é um campo obrigatório do decimal.


#### Integrar seu Backend com os dois Processadores de Pagamento

Seu backend deverá se integrar com dois Processadores de Pagamento. Os dois serviços possuem API idênticas, então as descrições aqui servem para os dois.
...
**resposta**
- `message` campo sempre presente do tipo texto.
...
**resposta**
- `failing` é um campo sempre presente do tipo booleano que indica se o endpoint **Payments** está disponível. Se não estiver, significa que requições para o endpoint receberão erros na faixa HTTP5XX.
...
#### Payment Details
Você não precisa se integrar com este endpoint. Ele serve para troubleshooting, caso precise.
```
GET /payments/{id}
...
```
**requisição**
- `delay` é um campo obrigatório do tipo inteiro para definir os milisegundos de atraso no endpoint **Payments**.
...
**resposta**
- `message` um campo sempre presente do tipo texto.


#### Resumo dos Endpoints

As tabelas abaixo oferem um resumo para facilitar o desenvolvimento.
...
| GET /payments-summary             | Devolve detalhes das requisições de processamento de pagamentos. |
...
| GET /payments/{id}                | Mostra detalhes duma requisição de processamento de pagamento. |
| GET /admin/payments-summary       | Devolve detalhes das requisições de processamento de pagamentos. |
...
A Rinha simula sistemas reais em uma escala micro (com diferenças muito importantes como pouca resilência, não escalabilidade, foco em performance extrema com pouco consumo de recursos, etc) e para tal as soluções devem usar containers com docker compose. Para essa edição, a solução completa (o backend que você desenvolverá e os Payment Processors) irá usar dois docker compose simulando redes diferentes. Você precisará criar o seu arquivo docker-compose.yml que irá se conectar a rede do [docker-compose.yml](/docker-compose.yml) dos Payment Processors.
...
Nas edições anteriores, havia restrições de tecnologia – por exemplo, apenas alguns bancos poderiam ser usados, load balancers, etc. Nessa edição, as restrições de tecnologia foram afrouxadas – qualquer tecnologia pode ser usada. Entretanto, as seguintes restrições* de containers devem ser obedecidas:
...
<sub><sup>* Qualquer outra restrição poderá ser adicionada ou removida ao decorrer do evento a critério do criador.</sup></sub>

Uma das essências da Rinha de Backend é a restrição de recursos computacionais. Essa restrições devem ser declaradas junto à declaração dos serviços (ou templates) em seu docker-compose e não poderão exceder o seguinte* (distribuído da forma que quiser entre todos os serviços declarados):

- 1.5 unidades de CPU
- 650MB de memória

<sub><sup>* Sujeito a mudanças ao decorrer do evento a critério do criador.</sup></sub>
...
Para ter seu backend testado oficialmente na Rinha de Backend, ver os resultados em comparação com as outras submissões e ter seu nome listado como participantes, você precisará fazer o seguinte:
...
- Ter suas imagens declaradas no docker-compose.yml disponíveis num registro público de imagens (https://hub.docker.com/ por exemplo). As imagens devem ser construídas para `linux/amd64` – isso é especialmente importante se você desenvolve em Mac ou Windows. Exemplo de como construir uma imagem docker para `linux/amd64`: `docker buildx build --platform linux/amd64 -t your-image-name:tag .`.
...
- Abrir um PR neste repositório adicionando um diretório com sua identificação em [participantes](participantes/). Neste PR você deverá:
    ...
    - Incluir o arquivo `docker-compose.yml` com suas dependências (scripts de banco, configurações, etc.).
    - Incluir um manifesto.json com a seguinte estrutura para facilitar a coleta das tecnologias usadas:
    ```json
    {
        "name": "Fernanda Giannella",
        "social": ["https://x.com/fer", "https://bsky.app/profile/fer.bsky.social"],
        "source-code-repo": "https://github.com/fernanda-giannella/rinha-de-backend-2025",
        "langs": ["java"],
...
A parte mais importante do processo da Rinha de Backend (fora o aprendizado e diversão) é o teste. Por isso, faz todo o sentido que você consiga testar seu backend mais ou menos da mesma forma que o teste final. Nas duas últimas edições, foi usada a ferramenta Gatling, mas muita gente pediu e sugeriu o [k6](https://k6.io/) e por isso nessa edição a ferramenta usada será o k6!!!
